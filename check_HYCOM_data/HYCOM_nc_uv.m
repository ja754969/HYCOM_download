...
%%
index_num = days(last_date-first_date)+1;
%%
data_folder = './';
u_HYCOM = [];
v_HYCOM = [];
...
%     HH = num2str(hour(the_date),'%02.0f');
    filename_u_i = [data_folder yyyy '-' MM '-' dd 'T12_water_u.nc'];
    filename_v_i = [data_folder yyyy '-' MM '-' dd 'T12_water_v.nc'];
    date_str(i,:) = [yyyy MM dd];
    disp(filename_u_i);
    lon = ncread(filename_u_i,'lon');
    lat = ncread(filename_v_i,'lat');
    u_i = ncread(filename_u_i,'water_u');
    v_i = ncread(filename_v_i,'water_v');
